...
from oauth2client.service_account import ServiceAccountCredentials #참가
...
def init():
	...
	try:
		...
		basicSetting.append(inputData[4][17:])     #basicSetting[11] : 참가 채널 ID
	...
	if basicSetting[10] !="":
		scope = ['https://spreadsheets.google.com/feeds', 'https://www.googleapis.com/auth/drive'] #참가
		credentials = ServiceAccountCredentials.from_json_keyfile_name(basicSetting[10], scope) #참가
...
class mainCog(commands.Cog): 
		...
		if basicSetting[7] == "":
			...
			if basicSetting[11] != "":
				if str(basicSetting[11]) in channel_id:
					print('< 참가채널 [' + ctx.guild.get_channel(int(basicSetting[11])).name + '] 접속완료>')
				else:
					basicSetting[11] = ""
					print(f"참가채널 ID 오류! [{command[28][0]} 참가] 명령으로 재설정 바랍니다.")
			if basicSetting[18] != "":
	...
	async def menu_(self, ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			command_list += ','.join(command[28]) + ' 사다리, 참가, 척살, 경주, 아이템\n'     #!채널설정
			command_list += ','.join(command[42]) + ' 사다리, 참가, 척살, 경주, 아이템\n'     #!채널삭제
	...
	async def setting_(self, ctx):	
		...
		if ctx.message.channel.id == basicSetting[7]:
			...
			if basicSetting[11] != "" :
				setting_val += '참가채널 : ' + self.bot.get_channel(int(basicSetting[11])).name + '\n'
			if basicSetting[18] != "" :
	...
	async def set_channel_(self, ctx):
		...
		if channel == basicSetting[7] and msg in ["사다리", "참가", "척살", "경주", "아이템"]:
		...
		if msg == '사다리' : #사다리 채널 설정
			...
			print(f'< 참가채널 [{ctx.message.channel.name}] 설정완료 >')
			return await ctx.send(f'< 참가채널 [{ctx.message.channel.name}] 설정완료 >', tts=False)
		elif msg == '척살' :
			inidata_textCH = repo.get_contents("test_setting.ini")
			file_data_textCH = base64.b64decode(inidata_textCH.content)
			file_data_textCH = file_data_textCH.decode('utf-8')
	...
	async def remove_channel_(self, ctx):
		...
		if msg == '사다리' : #사다리 채널 설정
			...
			print(f'< 참가채널 [{ch_name}] 삭제완료 >')
			return await ctx.send(f'< 참가채널 [{ch_name}] 삭제완료 >', tts=False)
		elif msg == '척살' :
			inidata_textCH = repo.get_contents("test_setting.ini")
			file_data_textCH = base64.b64decode(inidata_textCH.content)
			file_data_textCH = file_data_textCH.decode('utf-8')
...
class IlsangDistributionBot(commands.AutoShardedBot):
	...
	async def on_ready(self):
		...
		if str(basicSetting[7]) in channel_id:
			...
			if basicSetting[11] != "":
				if str(basicSetting[11]) in channel_id:
					print('< 참가채널 [' + self.get_channel(int(basicSetting[11])).name + '] 접속완료>')
				else:
					basicSetting[11] = ""
					print(f"참가채널 ID 오류! [{command[28][0]} 참가] 명령으로 재설정 바랍니다.")
			if basicSetting[18] != "":
